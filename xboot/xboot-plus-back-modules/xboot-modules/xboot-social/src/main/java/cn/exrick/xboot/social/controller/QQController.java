...
public class QQController {
    ...
    public String getAccessToken(@RequestParam(required = false) String code,
        ...
        // 存入数据库
        Social qq = socialService.findByOpenIdAndPlatform(openId, CommonConstant.SOCIAL_TYPE_QQ);
        if(qq==null){
            Social newqq = new Social().setOpenId(openId).setUsername(q.getNickname()).setAvatar(q.getFigureurl_1()).setPlatform(CommonConstant.SOCIAL_TYPE_QQ);
